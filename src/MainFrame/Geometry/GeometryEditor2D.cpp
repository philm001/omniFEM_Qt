            ...
            switch(numberOfLines - numberOfArcs)
            {
                case 0:// This is the case for if we have one arc and one line at the node
                {
                    ...
                    for(m = 0, k = 0; k < j; k++)
                    {
                        ...
                        if(shortestDistanceFromArc(I2[m], *connectedArc) < (radius / 10000.0) && calculateShortestDistance(QPointF(I1[m].getXComponent(), I1[m].getYComponent()), *connectedLine) < (radius / 10000.0) && Vabs(I1[m] - I2[m]) > (radius / 10000.0))
                        {
                            m++;
                            if(m == 2)
                                break; 
...
double geometryEditor2D::calculateShortestDistance(QPointF selectedPoint, edgeLineShape segment)
{
    double t, x[3], y[3];
